...
tasklist_t
tlist_sub (tasklist_t * a, tasklist_t * b)
{
  int i, j;
  tasklist_t result;
  result.ntasks = 0;
...
void
tasklist_init (tasklist_t * l)
{
  l->ntasks = 0;
}

int
tasklist_add (tasklist_t * l, int taskid)
{
...
int
rtmode_addtask (rtmode_t * g, int modeid, int tid)
{
  if (modeid >= g->nmodes)
    return 0;
  else
...
int
rtmode_taskfind (rtmode_t * g, int tid)
{
  int i;
...
void
maxsem_post (maxsem_t * gs, tspec * t)
{
...
tspec
maxsem_wait (maxsem_t * gs, int nsignals)
{
  pthread_mutex_lock (&gs->m);
