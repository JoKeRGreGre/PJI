...
void
ptask_init (int policy, global_policy global, sem_protocol protocol)
{
  int i;
...
static int
__create_internal (void (*task) (void), tpars * tp)
{
...
int
ptask_create_param (void (*task) (void), tpars * tp)
{
...
/*--------------------------------------------------------------*/
int
ptask_create (void (*task) (void), int period, int prio, int aflag)
{
...
pthread_t
ptask_get_threadid (int i)
{
  return _tid[i];
}

int
ptask_get_period (int i, int unit)
{
  int p;
...
void
ptask_set_period (int i, int period, int unit)
{
  pthread_mutex_lock (&_tp[i].mux);
...
int
ptask_get_deadline (int i, int unit)
{
  int d;
  pthread_mutex_lock (&_tp[i].mux);
...
void
ptask_set_deadline (int i, int dline, int unit)
{
  pthread_mutex_lock (&_tp[i].mux);
...
int
ptask_activate_at (int i, ptime offset)
{
...
ptime
ptask_get_nextactivation (int unit)
{
  ptime at_tmp;
...
void
ptask_syserror (char *f, char *msg)
{
  fprintf (stderr, "%s: ", f);
  fprintf (stderr, "%s \n", msg);
  exit (-1);
